...
export const Route = createFileRoute("/_authenticated/meal-planner")({
  head: () =>
    pageHead({
      ...
      description:
        "Generate personalized meal plans with Nova Vision AI. Get recipes, nutrition, and ingredient lists tailored to your goals and dietary preferences.",
    }),
...
function MealPlanner() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("profiles")
      .select("dietary_goal,diet_preference")
      .eq("id", user.id)
      .single()
      .then(({ data }) => {
      ...
      <div className="glass-card p-4 space-y-3">
        ...
        >
          {loading ? (
            ...
            <>
              <Sparkles className="size-4 mr-2" /> Generate today's plan
            </>
          )}
        </Button>
      </div>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="rounded-lg bg-muted p-2">
              <div className="font-bold">{m.nutrition?.protein_g ?? 0}g</div>
              <div className="text-muted-foreground">protein</div>
            </div>
            <div className="rounded-lg bg-muted p-2">
              <div className="font-bold">{m.nutrition?.carbs_g ?? 0}g</div>
              <div className="text-muted-foreground">carbs</div>
            </div>
            <div className="rounded-lg bg-muted p-2">
              <div className="font-bold">{m.nutrition?.fat_g ?? 0}g</div>
